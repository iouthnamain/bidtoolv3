...
**Stack:** Next.js App Router · tRPC · Drizzle/Postgres · TanStack Query
...
| Local procedure shell | `src/server/api/trpc.ts`                             |
...
3. Procedures validate input, then delegate to services in the local single-user context.
...
- REST routes beyond tRPC are minimal: health, version and catalog PDF file serving. Retired auth/setup endpoints return `410` for compatibility.
