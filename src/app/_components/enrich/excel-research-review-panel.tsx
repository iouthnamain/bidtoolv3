...
import { type ExcelResearchRowStatus } from "~/app/_components/research-enrich/excel-research-types";
        ...
        <div>
          ...
          <p className="mt-1 flex flex-wrap gap-1 text-xs text-slate-700">
            <span className="tabular-nums">
              {summary.total.toLocaleString("vi-VN")} dòng
            </span>
            <span className="tabular-nums">
      ...
      {!isDetailLoading && evidence.length > 0 ? (
        <div className="space-y-2">
          ...
          {evidence.map((item) => (
            <div
              key={item.id}
              className="flex gap-2 rounded border border-slate-500 bg-white p-2 text-xs shadow-[var(--shadow-flat)]"
            >
